...
See [[#command-interface][COMMAND INTERFACE]] and [[#key-names][Key names]] sections for more details on configuring keybindings.

See also =--input-test= for interactive binding details by key, and the [[#stats][stats]] built-in script
for key bindings list (including print to terminal). By default, the ? key toggles the display of this list.
...
- ? :: Toggle an overlay displaying the active key bindings.
  See [[#stats][STATS]] for more information.
- DEL :: Cycle OSC visibility between never / auto (mouse-move) / always
- ` :: Show the console. (ESC closes it again. See [[#console][CONSOLE]].)
...
- g-h :: Select a file from the watch history. Requires
  =--save-watch-history=.
- g-w :: Select a file from watch later config files
...
See [[#select][SELECT]] for more information.
...
=--vf=. They often require complicated escaping. See [[#video-filters][VIDEO FILTERS]] for details.
...
#+BEGIN_QUOTE
  ...
  1. Calling it as UNIX process. If you do this, /do not parse terminal
     ...
     gives you access to the [[#json-ipc][JSON IPC]] over unix domain sockets (or named pipes on Windows).
  ...
  2. Using libmpv. This is generally recommended when mpv is used as
     ...
     See [[#embedding-into-other-programs-libmpv][EMBEDDING INTO OTHER PROGRAMS (LIBMPV)]].

  3. As a user script ([[#lua-scripting][LUA SCRIPTING]], [[#javascript][JAVASCRIPT]], [[#c-plugins][C PLUGINS]]). This is
...
[[#options][OPTIONS]], [[#list-of-input-commands][List of Input Commands]], [[#properties][Properties]], [[#list-of-events][List of events]] (also see C API), [[#hooks][Hooks]].
...
else. See [[#pseudo-gui-mode][PSEUDO GUI MODE]].
...
More options to configure this functionality are listed in [[#watch-later][Watch Later]].
...
mpv has no official GUI, other than the OSC ([[#on-screen-controller][ON SCREEN CONTROLLER]]),
which is not a full GUI and is not meant to be. However, to compensate
for the lack of expected GUI behavior, mpv will in some cases start with
some settings changed to behave slightly more like a GUI mode.
...
- =--alang=<languagecode[,languagecode,...]>= :: Specify a prioritized
  ...
  This is a string list option. See [[#list-options][List Options]] for details.
...
- =--script=<filename>=, =--scripts=file1.lua:file2.lua:...= :: Load a
  ...
  =--scripts= is a path list option. See [[#list-options][List Options]] for details.
...
- =--use-filedir-conf= :: Look for a file-specific configuration file in
  the same directory as the file that is being played. See [[#file-specific-configuration-files][File-specific Configuration Files]].
...
- =--ytdl-raw-options=<key>=<value>[,<key>=<value>[,...]]= :: Pass
  ...
  This is a key/value list option. See [[#list-options][List Options]] for details.
...
- =--load-auto-profiles=<yes|no|auto>= :: Enable the builtin script that
  does auto profiles (default: auto). See [[#conditional-auto-profiles][Conditional auto profiles]] for details.
  =auto= will load the script, but immediately unload it if there are no conditional profiles.
...
- =--player-operation-mode=<cplayer|pseudo-gui>= :: For enabling "pseudo
  ...
  See [[#pseudo-gui-mode][PSEUDO GUI MODE]] for details.
...
- =--watch-history-path=<path>= :: The path in which to store the watch
  history. Default: =~~state/watch_history.jsonl= (see [[#paths][PATHS]]).
...
- =--vf=<filter1[=parameter1:parameter2:...],filter2,...>= :: Specify a
  ...
  [[#video-filters][VIDEO FILTERS]] for details and descriptions of the available filters. The option variants
...
- =--af=<filter1[=parameter1:parameter2:...],filter2,...>= :: Specify a
  ...
  [[#audio-filters][AUDIO FILTERS]] for details and descriptions of the available filters.
  The option variants =--af-add=, =--af-pre=, and =--af-clr= exist to modify
  a previously specified list, but you should not need these for typical use.
...
- =--audio-files=<files>= :: Play audio from an external file while
  ...
  This is a path list option. See [[#list-options][List Options]] for details.
...
- =--audio-exts=ext1,ext2,...= :: Audio file extensions to try to match
  ...
  This is a string list option. See [[#list-options][List Options]] for
...
- =--sub-files=<file-list>=, =--sub-file=<filename>= :: Add a subtitle
  ...
  =--sub-files= is a path list option (see [[#list-options][List Options]] for details),
  and can take multiple file names separated by =:= (Unix) or
...
- =--sub-filter-regex-...=...= :: Set a list of regular expressions to
  ...
  empty). This is a string list option. See [[#list-options][List Options]] for details.
  Normally, you should use =--sub-filter-regex-append=<regex>=,
...
- =--sub-fonts-dir=<path>= :: Font files in this directory are used by
  ...
  using fonts.conf (see [[#files][FILES]] section) to include additional mpv user settings.
...
- =--title=<string>= :: Set the window title. This is used for the video
  ...
  Properties are expanded. (See [[#property-expansion][Property Expansion]].)
...
- =--cdda-device=<path>= :: Specify the CD device for CDDA playback. The
  default device path depends on the OS. See the [[#optical-drives][OPTICAL DRIVES]] section.
...
- =--dvd-device=<path>= :: Specify the DVD device or .iso filename. You
  ...
  depends on the OS. See the [[#optical-drives][OPTICAL DRIVES]] section.
...
- =--bluray-device=<path>= :: Specify the Blu-ray disc location. Must be
  ...
  the OS. See the [[#optical-drives][OPTICAL DRIVES]] section.
...
- =--input-test= :: Input test mode. Instead of executing commands on
  ...
  OSD only, just like any other binding). See [[#input-conf][INPUT.CONF]].
...
- =--osd-msg1=<string>= :: Show this string as message on OSD with OSD
  ...
  changed (see =--osd-level=). Expands properties; see [[#property-expansion][Property Expansion]].
...
- =--osd-status-msg=<string>= :: Show a custom string during playback
  ...
  =Input Command Prefixes=). Expands properties. See [[#property-expansion][Property Expansion]].
...
- =--screenshot-dir=<path>= :: Store screenshots in this directory. This
  ...
  the directory from which mpv was started. In pseudo-gui mode
  (see [[#pseudo-gui-mode][PSEUDO GUI MODE]]), this is set to the desktop.
...
- =--term-status-msg=<string>= :: Print out a custom string during
  playback instead of the standard status line. Expands properties.
  See [[#property-expansion][Property Expansion]].
- =--term-title=<string>= :: Set the terminal title. Currently, this
  ...
  Expands properties. See [[#property-expansion][Property Expansion]].
...
- =--gpu-context=<context1,context2,...[,]>= :: Specify a priority list
  ...
  This is an object settings list option. See [[#list-options][List Options]] for details.
...
- =--image-exts=ext1,ext2,...= :: Image file extensions to try to match
  ...
  [[#list-options][List Options]] for details.
  Use =--help=image-exts= to see default extensions.- =--cover-art-whitelist=filename1,filename2,...= :: Filenames to load
...
- =--archive-exts=ext1,ext2,...= :: Archive file extensions to try to
  ...
  This is a string list option. See [[#list-options][List Options]] for details.
  Use =--help=archive-exts= to see the default extensions.
...
- =--playlist-exts=ext1,ext2,...= :: Playlist file extensions to try to
  ...
  This is a string list option. See [[#list-options][List Options]] for details.
  Use =--help=playlist-exts= to see the default extensions.
...
- =alsa= :: ALSA audio output driver.
  ...
  You can also try [[https://github.com/mpv-player/mpv/wiki/ALSA-Surround-Sound-and-Upmixing][using the upmix plugin]]. This setup enables multichannel
  audio on the =default= device with automatic upmixing with shared access,
  so playing stereo and multichannel audio at the same time will work as
  expected.
...
- =avfoundation= (macOS only) :: Native macOS audio output driver using
  =AVSampleBufferAudioRenderer= in AVFoundation, which supports [[https://support.apple.com/en-us/HT211775][spatial audio]].
...
- =gpu= :: General purpose, customizable, GPU-accelerated video output
  ...
  See [[#gpu-renderer-options][GPU renderer options]] for options specific to this VO.
...
- =gpu-next= :: Experimental video renderer based on =libplacebo=. This
  ...
  See [[#gpu-renderer-options][GPU renderer options]] for a list.
...
- =--af=...= :: Setup a chain of audio filters.
  See =--vf= ([[#video-filters][VIDEO FILTERS]]) for the full syntax.
...
described above in combination with the =vf= command (see [[#command-interface][COMMAND INTERFACE]])
to get more control over this. Initially disabled filters with =!= are
useful for this as well.

Note: to get a full list of available video filters, see =--vf=help= and
...
=command_name= is an unquoted string with the command name itself.
See [[#list-of-input-commands][List of Input Commands]] for a list.
...
or =$>=. See [[#input-command-prefixes][Input Command Prefixes]] and [[#property-expansion][Property Expansion]].
...
changed with the =expand-properties= prefix. See [[#input-command-prefixes][Input Command Prefixes]].
...
For complex commands, consider using [[#named-arguments][Named arguments]] instead, which should
give slightly more compatibility. Some commands do not support named arguments and inherently take an array, though.
...
Remember to quote string arguments in input.conf (see [[#flat-command-syntax][Flat command syntax]]).
...
- =seek <target> [<flags>]= :: Change the playback position. By default,
  ...
  input command prefix in [[#input-command-prefixes][Input Command Prefixes]] for details.
...
- =sub-seek <skip> [<flags>]= :: Change video and audio position such
  ...
  short prefetch range.
  See [[#cache][Cache]] for details on how to control the available prefetch range.
...
- =change-list <name> <operation> <value>= :: This command changes list
  options as described in [[#list-options][List Options]].
  The =<name>= parameter is the normal option name, while =<operation>=
  is the suffix or action used on the option.
...
- =loadfile <url> [<flags> [<index> [<options>]]]= :: Load the given
  ...
  restored to the previous value at end of playback (see [[#per-file-options][Per-File Options]]).
...
- =print-text <text>= :: Print text to stdout. The string can contain
  properties (see [[#property-expansion][Property Expansion]]). Take care to put the argument in quotes.
- =expand-text <text>= :: Property-expand the argument and return the
  ...
  a script using =mp.command_native=. (see [[#property-expansion][Property Expansion]]).
...
- =apply-profile <name> [<mode>]= :: Apply the contents of a named
  ...
  - =restore= :: Restore options set by a previous =apply-profile=
    ...
    nothing could be done. See [[#runtime-profiles][Runtime profiles]] for details.
...
- =overlay-add <id> <x> <y> <file> <offset> <fmt> <w> <h> <stride> <dw> <dh>= :: Add
  ...
  guaranteed, so you should always call them with named arguments, see [[#named-arguments][Named arguments]].
...
- =run <command> [<arg1> [<arg2> [...]]]= :: Run the given command.
  ...
  expanded like in [[#property-expansion][Property Expansion]].
...
- =subprocess= :: Similar to =run=, but gives more control about process
  ...
  see [[#named-arguments][Named arguments]].
...
- =screenshot-to-file <filename> [<flags>]= :: Take a screenshot and
  ...
  expansion as described in [[#property-expansion][Property Expansion]].
...
- =vf <operation> <value>= :: Change video filter chain.
  ...
  The semantics are exactly the same as with option parsing
  (see [[#video-filters][VIDEO FILTERS]]). As such the text below is a redundant and incomplete summary.
...
- =dump-cache <start> <end> <filename>= :: Dump the current cache to the
  ...
  was called to explicitly stop the command. See [[#synchronous-vs-asynchronous][Synchronous vs. Asynchronous]].
...
- =context-menu= :: Show context menu on the video window.
  See [[#context-menu][Context Menu]] section for details.
...
- =async= :: Allow asynchronous execution (if possible). Note that only
  ...
  returns. See [[#synchronous-vs-asynchronous][Synchronous vs. Asynchronous]].
...
- =menu-data= (RW) :: This property stores the raw menu definition.
  See [[#context-menu][Context Menu]] section for details.
...
- =input-key-list= :: List of [[#key-names][Key names]], same as output by =--input-keylist=.
...
used (see [[#flat-command-syntax][Flat command syntax]], [[#commands-specified-as-arrays][Commands specified as arrays]] and [[#named-arguments][Named arguments]]),
but it can always be enabled with the =expand-properties= prefix or disabled
with the =raw= prefix, as described in [[#input-command-prefixes][Input Command Prefixes]].
...
bound to a different key. See the [[#script-commands][Script Commands]] section.
...
described in [[#mp-options-functions][mp.options functions]].
...
=--script-opts= command-line option. The configuration syntax is described in [[#mp-options-functions][mp.options functions]].
...
- Ctrl+r :: Search the command history. See [[#select][SELECT]] for the key bindings in this mode.
...
- =history_path= :: Default: =~~state/command_history.txt=
  ...
  The file path for =persist_history= (see [[#paths][PATHS]]).
...
search of the presented items, and key bindings listed in [[#console][CONSOLE]] are extended with the
following:
...
starting with =g= listed in [[#keyboard-control][Keyboard Control]].
...
syntax is described in [[#mp-options-functions][mp.options functions]].
...
- =mp.command(string)= :: Run the given command. This is similar to the
  commands used in input.conf. See [[#list-of-input-commands][List of Input Commands]].

  By default, this will show something on the OSD (depending on the
  command), as if it was used in =input.conf=.
  See [[#input-command-prefixes][Input Command Prefixes]] how to influence OSD usage per command.
...
- =mp.del_property(name)= :: Delete the given property. See
  =mp.get_property= and [[#properties][Properties]] for more information about properties.
...
- =mp.get_property(name [,def])= :: Return the value of the given
  ...
  input.conf. See [[#properties][Properties]] for a list of properties. The returned string
  is formatted similar to =${=name}= (see [[#property-expansion][Property Expansion]]).
...
- =mp.set_property(name, value)= :: Set the given property to the given
  string value. See =mp.get_property= and [[#properties][Properties]] for more information
  about properties.
...
- =mp.register_event(name, fn)= :: Call a specific function when an
  ...
  See [[#events][Events]] and [[#list-of-events][List of events]] for details.
...
- =mp.add_periodic_timer(seconds, fn [, disabled])= :: Call the given
  ...
  instead of =.= (Refer to [[https://www.lua.org/manual/5.2/manual.html#3.4.9]] .)
...
- =mp.get_script_directory()= :: Return the directory if this is a
  script packaged as directory (see [[#script-location][Script location]] for a description).
...
For the existing event types, see [[#list-of-events][List of events]].
...
- =mp.add_hook(type, priority, fn)= :: Add a hook callback for =type= (a
  ...
  See [[#hooks][Hooks]] for currently existing hooks and what they do - only the hook
...
Currently, only "proper" commands (as listed by [[#list-of-input-commands][List of Input Commands]])
...
In addition to the commands described in [[#list-of-input-commands][List of Input Commands]], a few
...
- =set_property= :: Set the given property to the given value.
  See [[#properties][Properties]] for more information about properties.
...
config directory (see the [[#files][FILES]] section for details), or load them through
other means (see [[#script-location][Script location]]).
...
- The file [[https://github.com/mpv-player/mpv/blob/master/DOCS/interface-changes.rst]]
  documents changes to the command and user interface, such as options and properties.
...
See: [[https://github.com/mpv-player/mpv-examples/tree/master/cplugins]]
...
may be different. For Windows-specifics, see [[#files-on-windows][FILES ON WINDOWS]] section.
...
- =~/.config/mpv/mpv.conf= :: mpv user settings (see [[#configuration-files][CONFIGURATION FILES]] section)
- =~/.config/mpv/input.conf= :: key bindings (see [[#input-conf][INPUT.CONF]] section)
...
- =~/.config/mpv/fonts/= :: Default location for =--sub-fonts-dir=
  (see [[#subtitles][Subtitles]]) and =--osd-fonts-dir= (see [[#osd][OSD]]).
- =~/.config/mpv/scripts/= :: All files in this directory are loaded as
  ...
  See [[#script-location][Script location]] for details.
...
- =~/.config/mpv/script-opts/osc.conf= :: This is loaded by the OSC
  script. See the [[#on-screen-controller][ON SCREEN CONTROLLER]] docs for details.
...
Other config files (such as =input.conf=) are in the same directory.
See the [[#files][FILES]] section above.
